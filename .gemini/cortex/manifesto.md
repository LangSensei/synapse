...
- **Execution History (Neuron):** Every spike must have a unique neuron (`.gemini/neurons/{user}/spikes/{spike_id}/`) to maintain state and history.
...
2. **Context Setup:** Initialize or resume neuron at `.gemini/neurons/${alias}/spikes/${spike_id}/`.
3. **Cortex Sync:** 
...
4. **Structured Planning:** Invoke `planning-with-files` skill within the spike directory.
