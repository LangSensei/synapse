...
- **Strict Options Pattern**: Every interaction requiring user input must be structured as numbered options. Multiple lists are permitted for clarity, but the numbering must remain continuous and sequential across the entire response to prevent ambiguity.
...
- **Explicit Handoff Protocol**: The transition from 'execution' to 'completion' now requires an explicit interactive confirmation step.
